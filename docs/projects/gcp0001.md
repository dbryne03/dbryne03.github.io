...
**Infrastructure is provisioned with gcloud CLI.** GCS buckets, BigQuery datasets, Artifact Registry, and Secret Manager secrets are all managed by an idempotent shell bootstrap script (`infra/bootstrap.sh`) that checks existence before creating. The script runs automatically on every merge to main via GitHub Actions — nothing is provisioned manually through the console.
